...
public class ProgressHUD: UIView {
    ...
    private var fontStatus        = UIFont.init(name: "HelveticaNeueeTextPro-Bold", size: 24)
    ...
    private func setupBackground(_ interaction: Bool) {
        ...
        if (viewBackground == nil) {
            let topView =  UIWindow.topViewController(UIApplication.shared.windows.first!)
            viewBackground = UIView(frame: self.bounds)
            topView()?.view.addSubview(viewBackground!)
            topView()?.view.bringSubviewToFront(viewBackground!)
        }
